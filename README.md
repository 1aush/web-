...
本仓库包含多个基�?.NET 10.0 �?ASP.NET Core Web API 项目�?
---
...
| 项目 | 说明 | 状�?|
|------|------|------|
| [NetTask](#nettask任务管理系统) | 企业内部在线任务管理系统 | 已完�?|
| [NetFavorite](#netfavorite书签管理系统) | 书签管理系统 | 已完�?|
...
- **框架**: .NET 10.0
- **数据�?*: SQL Server + Entity Framework Core
- **认证**: JWT Bearer Token
...
�?  ├── DepartmentController.cs         # 部门CRUD（管理员权限�?�?  ├── LoginUserController.cs          # 用户CRUD（管理员权限�?�?  ├── TaskItemController.cs           # 个人任务CRUD
...
�?  └── PermissionAuthorizeAttribute.cs # 自定义特�?├── NetTaskDbContext.cs                 # 数据库上下文
├── Program.cs
└── appsettings.json
```
...
| 方法 | 路径 | 权限 | 说明 |
|------|------|------|------|
| GET | /api/DepartmentTaskItem | 部门任务列表 | 获取部门所有任�?|
...
浏览器访�?Swagger：http://localhost:5000/swagger

---

# NetFavorite（书签管理系统）

基于 .NET Core 10.0 的书签管理系统后�?API，支持用户认证、书签管理、分类管理等功能�?
## 功能特�?
### 用户管理
- 用户注册（自动加密密码）
- 用户登录（JWT Token 认证�?- 修改密码（验证旧密码，加密新密码�?- 用户信息查询与修�?- 用户删除

### 书签管理
- 书签 CRUD 操作（需认证�?- 用户只能操作自己的书签（通过 Bookmark_LoginUserId 关联�?- 自动记录创建时间

### 安全特�?- 密码加盐哈希加密（PBKDF2 + HMACSHA512�?0次迭代）
- JWT Token 认证
...
NetFavorite/
├── Controllers/
�?  ├── LoginController.cs        # 登录、修改密码、测试接�?�?  ├── LoginUserController.cs    # 用户管理
...
├── wwwroot/swagger-ui/           # Swagger 自定义资�?├── NetFavoriteDbContext.cs
├── Program.cs
└── appsettings.json
```
...
浏览器访�?Swagger：http://localhost:5248/swagger
...
| 包名 | 版本 | 用�?|
|------|------|------|
| Microsoft.EntityFrameworkCore.SqlServer | 10.0.5 | SQL Server 数据库访�?|
